...
export default function DisplayDigits(props) {
  ...
  if (data.length <= 0) {
    Papa.parse(
      "https://docs.google.com/spreadsheets/d/e/2PACX-1vRaT3afh6TliK1NPPeyA2pZUzp-tqVK3rwN-oN6KNLc7Y4udVPGIJ2HMPJ7EdQ-bQX9pe-CM7Etrv0a/pub?output=csv",
      {
        download: true,
        header: false,
        complete: (results) => {
  ...
  const BlankElement = () => "";
  const DemoData = () => {
    return searchPossibilities.map((item) => (
      <li className="guide" key={`DemoData` + item + Math.random()}>
        {item}
      </li>
    ));
  };
      ...
      return data.map((item, ind) => {
        ...
        if (found !== undefined) {
          ...
          if (checkDays) {
            ...
            if (searchData !== "000" && searchData.length > 0) {
              let prevDay = 30;
  ...
  if (
    ...
    DisplayGroup = () => (
      <>
        {searchDataGroup.map((result) => (
          <ul className="Group" key={`Group` + Math.random()}>
            {result.group.map((res) => {
              ...
              return (
                <li key={`GroupLI` + res + Math.random()} className={found}>
                  {res}
                </li>
              );
            })}
